...
<template>
    ...
    <aside class="bg-background flex flex-col h-full w-full">
      <div class="w-full flex justify-between h-14 bg-background items-center">
        <TopBarIcon tooltipText="关闭边栏" @click="sidebarStore.toggle()">
          <MobileSidebarIcon v-if="isMobile" class="size-5 opacity-60" />
          <DesktopSidebarIcon v-if="!isMobile" class="size-5 opacity-60" />
        </TopBarIcon>
  ...
  <Sheet aria-describedby="undefined" v-model:open="isOpen" v-if="isMobile">
